...
/* readCell: read in unit cell parameters, 
 *           and some simulation conditions */
unitCell readCell(void)
{
  ...
  for (i = 0; i < Npos; i++) {
    fscanf(f, " %d %d %f %f %f ", &c[i].n, &c[i].state, 
           &c[i].x, &c[i].y, &c[i].z);
    for (j = 0; j < 6; j++) {
      fscanf(f, "%d %d %d %d ", &c[i].nbr[j].n, &c[i].nbr[j].a, 
             &c[i].nbr[j].b, &c[i].nbr[j].c);
    }
  }
  c[Npos].n = -1;
  return c;
