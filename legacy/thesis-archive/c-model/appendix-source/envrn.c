...
int check300(Lattice lattice, int site)
{
  ...
  y = (lattice[si1].state - 201) + 
      (lattice[si2].state - 201) - x;  /* 300s */
...
int check500(Lattice lattice, int site)
{
  ...
  y = (lattice[al1].state - 101) + (lattice[al2].state - 101);
  if (lattice[p].state == 502 ||     /* subtract out doubly 
                                        counted bridges */
      lattice[p].state == 403 ||
      lattice[p].state == 405)
    y--;
...
int isActive(int site, Lattice lattice, int rxn)
{
  ...
  if (rxn < NHYD && rxn % 2 == 0) {   /* hydrolysis
    ...
    for (i = 0; (nbr = lattice[site].nbr[i]) >= 0 && !result; i++)
      for (j = 0; (nbr2 = lattice[nbr].nbr[j]) >= 0 && 
                  j < 6 && !result; j++)
    ...
    result = FALSE;                    /* must be at least one occupied 
                                        * nbr */
