...
- 2026-09-18 00:53 PDT — (hermes-custom-build-001; profile=workstation) — A2e DONE: 16/16 hash-bound GPU single points converged on the unchanged A2a structures. Skala-1.1 barriers are 143.632106 (def2-TZVP) and 145.076794 kJ/mol (def2-TZVPD) versus the 132.960133 focal point, crossing the pre-declared rejection boundary for this route; one reaction is not general validation. One cold review independently reproduced the result and its verifier-tamper finding is closed; full QA is 935 passed. Evidence: `docs/program/results/A2e-skala-functional-spike.md`.

- 2026-09-17 23:15 PDT — fable — A2e READY: filed a bounded Skala-1.1 functional spike (Microsoft Research's neural XC functional — MIT, native GPU4PySCF 1.8.1, resolves against quarry's exact pins with torch as the only new dependency) as 16 single points on A2a's four hash-bound si-neutral structures against the banked 132.960133 kJ/mol CC focal barrier, with pre-declared verdict bands and POLICY §12 limits. Motivation and the rest of the MSR AI-for-Science scan (reaction-capable MLIPs noted, MatterGen→petra as a post-M5 platform-variant idea, nothing else relevant) are in `docs/scoping/msr-ai4science-scan.md`. Docs only; no compute run; Victor authorized the spike 2026-09-17.
...
- 2026-09-14 00:19 PDT — (hermes-macbot-one; profile=laptop) — A9b mechanism reachability DONE with a corrected live-graph verdict: exact-deck replay proves 260/320 original-lattice release paths; 60 periodic boundary targets are structural NO-GOs (20 Al at `l4`, 20 Si at `oh2`, 20 Si at `oh1`). The hash-bound 1,020-event path report, reservoir-lineage exclusion, RNG-atomic likelihood weighting, and bounded importance-sampling runner are verified; zero observations remain censored/unrankable. Evidence: `docs/program/results/A9b-mechanism-reachability.md`.
- 2026-09-13 23:32 PDT — (hermes-macbot-zero; profile=laptop) — A9b RESERVOIR CONTRACT DONE: the historical `1e-30` sink is replaced for future runs by an executable, hash-bound pH 3/4/5 constant-activity open-flow materializer; receipts name sources, assumptions, selected activities, and boundary semantics. Deterministic Si+Al fixtures preserve lattice/reservoir lineage across adsorb → surface-state → desorb and count only original-lattice release. No calibrated rate or sensitivity result is claimed; ranking remains blocked on mechanism reachability plus the independent Verify edge. See `docs/program/results/A9b-reservoir-origin-contract.md`.

- 2026-09-13 20:52 PDT — (hermes-macbot-zero; profile=laptop) — A9 DONE by scope ruling with an honest partial/negative survey result: the 29 × 8 campaign observed zero original-lattice Si/Al release, the corrected runner classifies the sampled mechanism unresolved, ranked-response stationarity is not established, and all seven families are censored/unrankable. It names no top-k and no irrelevant family; the `1e-30` sink is explicitly not a realistic pH 3–5 reservoir. READY `A9b-mechanism-reachability` and `A9b-reservoir-origin-contract` own those gaps; workstation `A9b-sensitivity-ranking` blocks on both and now gates A3/A2/A2b. Evidence: `docs/program/results/A9-approximate-rate-closure.md`.
