export class WasmSim {
    ...
    header_json() {
        ...
        let deferred1_1;
        try {
            const ret = wasm.wasmsim_header_json(this.__wbg_ptr);
            deferred1_0 = ret[0];
            deferred1_1 = ret[1];
     ...
     */
    n_sites() {
        const ret = wasm.wasmsim_n_sites(this.__wbg_ptr);
        return ret >>> 0;
    }
    /**
     * Compile a deck from TOML text and build the engine. `seed`
     * overrides the deck's seed when >= 0.
     * @param {string} deck_toml
     * @param {number} seed
     */
    constructor(deck_toml, seed) {
        const ptr0 = passStringToWasm0(deck_toml, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len0 = WASM_VECTOR_LEN;
        const ret = wasm.wasmsim_new(ptr0, len0, seed);
        if (ret[2]) {
    ...
    snapshot_json() {
        ...
        let deferred1_1;
        try {
            const ret = wasm.wasmsim_snapshot_json(this.__wbg_ptr);
            deferred1_0 = ret[0];
            deferred1_1 = ret[1];
            return getStringFromWasm0(ret[0], ret[1]);
        } finally {
    ...
    step_batch(n) {
        ...
        let deferred1_1;
        try {
            const ret = wasm.wasmsim_step_batch(this.__wbg_ptr, n);
            deferred1_0 = ret[0];
            deferred1_1 = ret[1];
            return getStringFromWasm0(ret[0], ret[1]);
        } finally {
     ...
     */
    step_count() {
        const ret = wasm.wasmsim_step_count(this.__wbg_ptr);
        return ret;
    }
    /**
     ...
     */
    stop_reason() {
        const ret = wasm.wasmsim_stop_reason(this.__wbg_ptr);
        let v1;
        if (ret[0] !== 0) {
     ...
     */
    time() {
        const ret = wasm.wasmsim_time(this.__wbg_ptr);
        return ret;
    }
}
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('petra_wasm_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
